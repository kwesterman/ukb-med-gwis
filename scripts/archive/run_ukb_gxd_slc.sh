...
/GEM/GEM \
	--bgen /geno_dir/slc_genotypes_chr${chr}.bgen \
	--sample /geno_dir/slc_genotypes_chr${chr}.sample \
	...
	--out /res_dir/${pheno}_minimal_slc_chr${chr}

if [[ "$chr" -eq 1 ]]; then
	head -1 /res_dir/${pheno}_minimal_slc_chr${chr} >> /res_dir/${pheno}_minimal_slc
fi

tail -n +2 /res_dir/${pheno}_minimal_slc_chr${chr} >> /res_dir/${pheno}_minimal_slc
